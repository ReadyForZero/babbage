...
(ns babbage.provided.core
  (:require [babbage.core :as stats])
  (:use [babbage.core :only [defstatfn]]
        [babbage.monoid :only [monoid]]
        babbage.util
        [trammel.core :only [defconstrainedfn]])
  (:refer-clojure :exclude [max min count set list complement]))

(defstatfn max (monoid clojure.core/max Double/NEGATIVE_INFINITY))
(defstatfn min (monoid clojure.core/min Double/POSITIVE_INFINITY))
(defstatfn prod (monoid * 1))
